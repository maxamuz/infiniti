...
/**
 * Перенаправление после отправки Contact Form 7
 */
add_action('wp_footer', 'infiniti_cf7_redirect_script');

function infiniti_cf7_redirect_script()
{
?>
	<script>
		document.addEventListener('wpcf7mailsent', function(event) {
			...
			if (event.detail.contactFormId) {
				// Сохраняем URL текущей страницы в sessionStorage
				sessionStorage.setItem('cf7_return_url', window.location.href);

				// Перенаправляем на страницу "Спасибо"
				window.location.href = 'https://infmservice.ru/thanks/';
			}
		}, false);

		// Проверяем, находимся ли мы на странице "Спасибо"
		if (window.location.pathname === '/thanks/' || window.location.href.includes('/thanks/')) {
			// Получаем URL возврата
			var returnUrl = sessionStorage.getItem('cf7_return_url');

			if (returnUrl) {
				// Через 10 секунд возвращаемся обратно
				setTimeout(function() {
					window.location.href = returnUrl;
				}, 10000); // 10000 миллисекунд = 10 секунд
			} else {
				// Если нет URL возврата, возвращаемся на главную
				setTimeout(function() {
